...
class Tensor {
  public:
    Tensor(const std::vector<int>& shape) : shape_(shape), data_(std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<int>())) {}

    Tensor(const std::vector<int>& shape, const std::vector<T>& data) : shape_(shape), data_(data) {

...
class Task13(Task):
    ...
    def run(self):
        # def async_task():
        act = self.secllm_cpp_wrapper.BookKeeperLoad_Int8(self.layer_idx, self.task_id, 0)
        if MEASURE_TIME_WITH_NVTX == True:
            nvtx.range_push(self.to_string_info())
        act = act.to('cuda:0')
        torch.cuda.synchronize()
...
class Task39(Task):
    ...
    def run(self):
        ...
        assert attn_weights.dtype == torch.int32
...
class Task40(Task):
    ...
    def run(self):
        ...
        if MEASURE_TIME_WITH_NVTX == True:
            nvtx.range_pop()
        self.secllm_cpp_wrapper.BookKeeperStore_Int32(self.layer_idx, self.next_task_ids[0], 0, attn_weights)
        # threading.Thread(target=async_task).start()
...
class Task55(Task):
    ...
    def run(self):
        ...
        assert attn_output.dtype == torch.int32
...
class Task56(Task):
    ...
    def run(self):
        ...
        if MEASURE_TIME_WITH_NVTX == True:
            nvtx.range_pop()
        self.secllm_cpp_wrapper.BookKeeperStore_Int32(self.layer_idx, self.next_task_ids[0], 0, attn_output)
        # threading.Thread(target=async_task).start()

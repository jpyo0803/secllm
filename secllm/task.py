...
class Task16(Task):
    ...
    def run(self):
        ...
        weight = weight.to(torch.int32).transpose(-2, -1)
        ...
        with self.stream:
            ...
            weight_T_cupy = cupy.from_dlpack(weight)
...
class Task38(Task):
    ...
    def run(self):
        act_cpu = self.secllm_cpp_wrapper.BookKeeperLoad_Uint32(self.layer_idx, self.task_id, 0)

        with torch.cuda.stream(self.stream):
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_push(self.to_string_info())
            act_gpu = act_cpu.to('cuda:0', non_blocking=True)
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_pop()
        ...
        dst = GetBookKeeperLinearIndex(self.layer_idx, self.next_task_ids[0], 1)
        self.model.tensor_buffer[dst] = (act_cpu, act_gpu)
        # threading.Thread(target=async_task).start()
...
class Task39(Task):
    ...
    def run(self):
        ...
        k_cpu, k_gpu = self.model.tensor_buffer[k_loc]
        ...
        assert k_gpu.dtype == torch.uint32
        assert q.shape[-1] == k_gpu.shape[-1]
        ...
        if past_key_value is not None:
            assert not k_cpu.is_cuda
            _ = past_key_value.update_key(k_cpu, self.layer_idx)
...
class Task40(Task):
    ...
    def run(self):
        ...
        self.secllm_cpp_wrapper.BookKeeperStore_Uint32(self.layer_idx, self.next_task_ids[0], 0, tensor_copy_cpu)
        self.model.tensor_buffer[loc] = None
        # threading.Thread(target=async_task).start()
...
class Task55(Task):
    ...
    def run(self):
        ...
        v_cpu, v_gpu = self.model.tensor_buffer[v_loc]
        ...
        assert v_gpu.dtype == torch.uint32

        past_key_value = self.model.layers[self.layer_idx].past_key_value
        if past_key_value is not None:
            assert not v_cpu.is_cuda
            _ = past_key_value.update_value(v_cpu, self.layer_idx)

        if v_cache is not None:
            v_gpu = torch.cat([v_cache, v_gpu], dim=-2)

        assert p.shape[-1] == v_gpu.shape[-2]
...
class Task60(Task):
    ...
    def run(self):
        with torch.cuda.stream(self.stream):
            ...
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_push(self.to_string_info())
            self.model.layers[self.layer_idx].o_proj_weight_buffer = self.model.layers[self.layer_idx].o_proj_weight_buffer.to('cuda:0', non_blocking=True)
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_pop()
...
class Task64(Task):
    ...
    def run(self):
        ...
        with self.stream:
            ...
            act_cupy = cupy.from_dlpack(act)
            weight_cupy = cupy.from_dlpack(weight)
...
class Task72(Task):
    ...
    def run(self):
        with torch.cuda.stream(self.stream):
            ...
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_push(self.to_string_info())
            self.model.layers[self.layer_idx].gate_proj_weight_buffer = self.model.layers[self.layer_idx].gate_proj_weight_buffer.to('cuda:0', non_blocking=True)
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_pop()
...
class Task73(Task):
    ...
    def run(self):
        with torch.cuda.stream(self.stream):
            ...
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_push(self.to_string_info())
            self.model.layers[self.layer_idx].up_proj_weight_buffer = self.model.layers[self.layer_idx].up_proj_weight_buffer.to('cuda:0', non_blocking=True)
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_pop()
...
class Task89(Task):
    ...
    def run(self):
        with torch.cuda.stream(self.stream):
            ...
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_push(self.to_string_info())
            self.model.layers[self.layer_idx].down_proj_weight_buffer = self.model.layers[self.layer_idx].down_proj_weight_buffer.to('cuda:0', non_blocking=True)
            if MEASURE_TIME_WITH_NVTX == True:
                nvtx.range_pop()

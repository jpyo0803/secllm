...
class TestCppWrapper:
  ...
  @classmethod
  def Matmul_CPU(cls, x : torch.tensor, y : torch.tensor):
    # assert x.dim() == 4
    # assert y.dim() == 4
    ...
    cls.lib.Test_Matmul(cast(out.data_ptr(), POINTER(c_int32)), cast(x.data_ptr(), POINTER(c_int8)), cast(y.data_ptr(), POINTER(c_int8)), c_int(X_B * X_M), c_int(X_K), c_int(Y_K), c_int(X_N))
    return out
